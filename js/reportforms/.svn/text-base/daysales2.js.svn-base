Ext.ux.RmaGrid = Ext.extend(Ext.ux.ComlGrid, {
	...
	createScanWin:function(){
		...
		if(form.isValid()) {
			this.getStore().load({
				params:{start:0,limit:this.pagesize,Sales_account:form.findField('Sales_account').getValue(),starttime:form.findField('starttime').getValue().format('Y-m-d'),endtime:form.findField('endtime').getValue().format('Y-m-d')}
			});
		}
	},
