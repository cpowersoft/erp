Ext.ux.GoodsView = Ext.extend(Ext.Viewport, {
	...
	doSelect:function(id,info,action){
		if(!info){
                ...
                var tpl10 = new Ext.Template(
					...
					'<input type="text" placeholder="请选择分类" size="80" id="cat_name" value="{ali_cat_name}"/>',
					'<input type="hidden" id="cat_id" value="{ali_cat_id}"/>',
					'<button onclick="getAliCategoryFormTree(\'index.php?model=aliexpress&action=getcattree&com=0\',0,afterselect);" type="button">选择类目</button>',
					'&nbsp;&nbsp;<button onclick="uploadgood()" type="button">一键刊登</button></p></div></div>'
                );
		switch(id){
				case 'tab1':
	...
	var tpl = new Ext.XTemplate(
        ...
        items:[{
				...
				id:'tab10',
                title: 'Aliexpress刊登',
				disabled:(action[18]==0)?true:false,
				listeners: {activate: function(){
...
function joinupload(id){
	if(Ext.get('cat_id_4').getValue()){
		var grid = Ext.getCmp("goodgrid").getSelectionModel().getSelected();
		var cat_id = Ext.get('cat_id_4').getValue();
		Ext.Ajax.request({
