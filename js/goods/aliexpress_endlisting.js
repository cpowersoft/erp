		...
		header: '商品货号',
			...
			items:[{ 
				...
				handler: function(grid, rowIndex, colIndex) {
                    ...
                    parent.openWindow(id, '编辑Aliexpress产品', 'index.php?model=goods&action=EditALIGoods&status=2&id=' + id, 1000, 600)
				}
			},{
				icon: 'themes/default/images/upload.png',
				tooltip: '上架',
				iconCls: 'iconpadding',
				handler: function(grid, rowIndex, colIndex) {
        ...
        return Ext.create('Ext.toolbar.Toolbar', {
            items: [{
                ...
                handler: function() {
					...
					if(!ids)return false;
					openWindow(ids,'批量上架产品','index.php?model=aliexpress&action=onlineshelf&id='+ids,820,300,0);
                }
            },'->',/*{
			xtype:'combo',
                    ...
                    keypress: function(field, e) {
                        if (e.getKey() == 13) {
                            ...
                            function(store, options) {
                                var new_params = {
                                    keyword: Ext.getCmp('keyword').getValue(),
                                    user_id: Ext.getCmp('account').getValue(),
                                };
                                Ext.apply(store.proxy.extraParams, new_params)
                            });
                            store.load({
