Ext.define('Ext.ux.RefundGrid',{
	...
	createTbar:function(){
		...
		{text:'编辑',id:'editBtn',iconCls:'x-tbar-update',ref:'../editBtn',disabled:true,handler:this.addItem.bind(this,['edit'])},
		{text:'删除',id:'removeBtn',iconCls:'x-tbar-del',ref:'../removeBtn',disabled:true,handler:this.removeRecord.bind(this)},
	...
	addItem:function (action) {
		if (action=='edit') {
			...
			parent.openWindow(id, '编辑付款单', this.addURL+'&id='+id,300,330);
		} else {
			parent.openWindow(id, '添加付款单', this.addURL,300,330);
		}
	},
	confirms:function(status){
		...
		Ext.Msg.confirm('提示!','是否确定?',function(btn){
			if(btn=='yes'){
				Ext.Ajax.request({
					url:'index.php?model=finance&action=updateConfirm&id='+r.data.rec_id+'&status='+status+'&deal_type='+1,
