Ext.ux.OrdergoodsGrid = Ext.extend(Ext.grid.GridPanel, {
    ...
    createTbar: function() {
        ...
        this.tbar = ['SKU',{
						...
						handler:function(){
							window.open().location.href = 'index.php?model=Statistics&action=exportordergoods&goods_sn='+Ext.fly('goods_sn').dom.value+'&starttime='+Ext.fly('starttime').dom.value+'&totime='+Ext.fly('totime').dom.value+'&account='+Ext.getCmp('account').getValue()+'&num='+Ext.fly('num').dom.value+'&currency='+Ext.getCmp('currency').getValue()+'&timetype='+Ext.getCmp('timetype').getValue();
							}
					}];
    },
