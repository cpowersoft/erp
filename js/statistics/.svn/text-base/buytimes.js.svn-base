Ext.ux.buytimeGrid=Ext.extend(Ext.grid.GridPanel,{
	initComponent:function(){
		...
		this.createTbar();
		this.createColumns();
		Ext.ux.buytimeGrid.superclass.initComponent.call(this);
	},
	...
	createTbar:function(){
        ...
        this.tbar = ['销售帐户:',{xtype:'combo',
						...
						handler:function(){
							window.open().location.href='index.php?model=statistics&action=exportBuytimes&Sales_account='+Ext.getCmp('Sales_account').getValue()+'&starttime='+Ext.fly('starttime').dom.value+'&totime='+Ext.fly('totime').dom.value;
						}
	 				}];
	} ,
