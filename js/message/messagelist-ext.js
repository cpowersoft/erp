Ext.ux.MessageView = Ext.extend(Ext.Viewport, {
    ...
    createStore: function() {
        ...
        reader: new Ext.data.JsonReader({
            ...
            id: 'id'
			},['id','messageid','SenderID','sender_mail','body','subject','itemid','itemtitle','goods_img','answer','accountid','CreationDate','answerstatus','ebay_ack','itemEndTime','msg_type','has_history','note'])
    	});			
    ...
    createorderStore: function() {
		...
		proxy : new Ext.data.HttpProxy({url:this.orderURL}),
        reader: new Ext.data.JsonReader({
			root: 'topics',
            totalProperty: 'totalCount',
            id: 'order_id'
			},['order_id','order_sn','track_no','order_status','end_time','street1','street2','consignee','city','state','zipcode','country','tel'])
    	});			
    },
	...
	creatTree:function(){
		...
		function treeClick(node,e) {
			 if(node.isLeaf()){
				...
				store.load({
					params:{start:0,accountid:node.id,limit:20},
					scope:this.store
					});
			 }
		};
			...
			items:['-','更改分类:',type_comb,'-','更改状态:',type_comb2,'-',
				   {
					   ...
					   id: 'sendmsg',
					   disabled: true,
					   handler:this.sendMsg.createDelegate(this)
					}
				   ]
        });
    },
