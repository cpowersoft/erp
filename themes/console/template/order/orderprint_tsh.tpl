...
<!--{elseif $type eq 161}--><!-- 中国邮政小包（挂号）10*10 -->
<!--{foreach from =$orders item = order name=orderitem}-->
<div style="height:15px;overflow:hidden"></div>
 <DIV style="float:left;PADDING-BOTTOM: 0px; width:350px;MARGIN: 0px; PADDING-LEFT: 0px;PADDING-RIGHT: 0px; PADDING-TOP: 0px">
<!--地址标签-->
<ul>
<li>
...
<li><br><span style="font-size:12px;">`
<span><!--{$order.total}-->---<br>
<!--{foreach from =$order.goodsarr item = goods name=goodsitem}}-->
<!--{$goods.goods_sn}--><!--{if $goods.goods_qty > 1}-->&<!--{$goods.goods_qty}--><!--{/if}-->,
<!--{/foreach}--></span>
...
<!--地址标签-->
<div style="float:left;padding:0px;margin-left:10px;position:relative;top:0.3cm;width:7.2cm;height:8.9cm;border:1px solid #000;font-family:arial;font-size:12px;">
...
<tr style="padding:0px;margin:0px;height:25px;">
                        ...
                        overflow:hidden;padding:0px;margin:0px;width:1.3cm;font-size:6pt;text-align:center;">
                            <span  contenteditable = "true"><!--{$goods.Declared_weight}--></span>
                        </td>
                        <td style="padding:2px;    border-right:1px solid #000;       border-collapse:collapse;
                        overflow:hidden;padding:0px;margin:0px;width:1.5cm;font-size:6pt;text-align:center;">
                            <span  contenteditable = "true">$
...
<div style="clear:both;height:20px;" <!--{if !$smarty.foreach.orderitem.last && !$smarty.foreach.orderitem.first && ($smarty.foreach.orderitem.index+1) % 3 == 0}-->class="pagebreak"<!--{/if}-->></div>
<!--{/foreach}-->
<!--{elseif $type eq 99}-->
<!--{foreach from =$orders item = order name=orderitem}-->
